...
public final class TextExtractor {
	...
	public static String getContent(String f) throws Exception {
        Metadata metadata = new Metadata();
        StringWriter writer = new StringWriter();
		TextExtractor te = new TextExtractor(new XHTMLContentHandler(new WriteOutContentHandler(writer), metadata), metadata);
    ...
    private void processControlWord(int param, PushbackInputStream in) throws IOException, SAXException, TikaException {
        ...
        if (inHeader) {
            if (equals("ansicpg")) {
                ...
                metadata.add(Property.internalInteger(
								 "meta" + Metadata.NAMESPACE_PREFIX_DELIMITER + "page-count"), Integer.toString(param));
            } else if (equals("nofwords")) {
                metadata.add(Property.internalInteger(
								 "meta" + Metadata.NAMESPACE_PREFIX_DELIMITER + "word-count"), Integer.toString(param));
            } else if (equals("nofchars")) {
                metadata.add(Property.internalInteger(
								 "meta" + Metadata.NAMESPACE_PREFIX_DELIMITER + "character-count"), Integer.toString(param));
            } else if (equals("yr")) {
                year = param;
            } else if (equals("mo")) {
                month = param;
        ...
        if (inHeader) {
            ...
            if (uprState == -1) {
                ...
                if (equals("author")) {
                    ...
                    nextMetaData = Property.externalText(
						"cp" + Metadata.NAMESPACE_PREFIX_DELIMITER + "category");
                } else if (equals("comment")) {
                    nextMetaData = TikaCoreProperties.COMMENTS;
                } else if (equals("company")) {
                    nextMetaData = Property.externalText("extended-properties" + Metadata.NAMESPACE_PREFIX_DELIMITER + "Company");
                } else if (equals("manager")) {
                    nextMetaData = Property.externalTextBag(
						"extended-properties" + Metadata.NAMESPACE_PREFIX_DELIMITER + "Manager");
                } else if (equals("template")) {
                    nextMetaData = Property.externalText(
						"extended-properties" + Metadata.NAMESPACE_PREFIX_DELIMITER + "Template");
