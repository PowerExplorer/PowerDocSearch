...
import android.util.*;
import android.text.*;
import net.gnu.agrep.SlidingTabsFragment;
...
public class SlidingHorizontalScroll extends HorizontalScrollView {
    ...
    private final SlidingTabStripLinearLayout mTabStrip;
    ...
    public SlidingHorizontalScroll(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTabStrip = new SlidingTabStripLinearLayout(context);
			...
			for (int i = 0; i < childCount; i++) {
				if (v == mTabStrip.getChildAt(i)) {
					if (currentItem != i) {
						...
						final PopupMenu popup = new PopupMenu(SlidingHorizontalScroll.this.getContext(), v);
						...
						popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
								public boolean onMenuItemClick(MenuItem item) {
									...
									if (R.id.close == itemId) {
										...
										if (count < 16) {
											fra.addTab(null);//, "utf-8",false, '\n');
										} else {
											Toast.makeText(getContext(), "Reached maximum number of tabs", Toast.LENGTH_SHORT).show();
										}
									}
									return true;
								}
